...
function tag_push() {
  ...
  tag=$1
  docker tag admin-console-deploy $tag
  docker push $tag
}
...
function deploy() {
  ...
  # Build admin-console-deploy
  make docker-image-deploy

  TAG=$(echo $GIT_COMMIT | cut -c1-${DEVSHIFT_TAG_LEN})

  if [ "$TARGET" = "rhel" ]; then
    tag_push ${REGISTRY}/openshiftio/rhel-fabric8-services-admin-console:$TAG
    tag_push ${REGISTRY}/openshiftio/rhel-fabric8-services-admin-console:latest
  else
    tag_push ${REGISTRY}/openshiftio/fabric8-services-admin-console:$TAG
    tag_push ${REGISTRY}/openshiftio/fabric8-services-admin-console:latest
  fi
